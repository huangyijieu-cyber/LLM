...
$$
r_t = - \beta \log \frac{\pi_\theta(a_t|s_t)}{\pi_{ref}(a_t|s_t)}, \quad t < T
$$

当生成最后一个词时:

$$
r_T = R_\phi(x, y) - \beta \log \frac{\pi_\theta(a_T|s_T)}{\pi_{ref}(a_T|s_T)}
$$
...
- $\log \frac{\pi_\theta}{\pi_{ref}}$ **(KL 散度惩罚项)**: 如果 Actor 生成的词偏离 Reference 太远, 这个值会变大. 乘以系数 $-\beta$ 后, 就会变成负分(扣分).
